...
pipeline {
   agent {
    ...
    environment {
        IDENTIFIER = 'btq-chs-poc.btq.sealights.co'
        tag = "chs-poc_${params.tag}"
    }
     stages {
                        ...
                        IP = "${IDENTIFIER}"
                                stage("Updating Helm") {
                                    sh script: """
                                        ...
                                        ssh -o StrictHostKeyChecking=no -i ${SSH_KEY_FILE} ${SSH_USER}@internal-chs-poc.btq.sealights.co 'bash /opt/sealights/install-btq.sh --tag=${env.tag} --buildname=${params.buildname} --labid=${params.labid} --branch=${params.branch} --token=${env.SL_TOKEN} --sl_branch=${params.branch}'
                                    """
                                }
                    }
                }
